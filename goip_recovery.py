...
def recovery_fun():
    ...
    # 点击系统设置
    Browser.find_element_by_id("H7").click()
    # 点击系统升级或恢复
    ...
    # 点击恢复出厂设置按钮
    Browser.find_element_by_id("load").click()

    # 判断弹出内容，并处理
    result = expected_conditions.alert_is_present()
    if result:
        result = Browser.switch_to.alert
        print(result.text)
        if "确认将网关恢复至出厂设置吗" or "Are you sure to retore the gateway to factory settings" in result.text:
            result.accept()
    else:
        print("alert 未弹出")
